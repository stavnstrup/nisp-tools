...
profiles (NISP), and is intended for resolving volume 4.
...
<xsl:template match="servicearea">
  ...
  <tgroup cols="4">
    ...
    <colspec colwidth="20*"/>
    <colspec colwidth="30*"/>
    <colspec colwidth="30*" colname="c4"/>
    <thead>
      <row>
        <entry>SUBAREA / SERVICE CATEGORY</entry>
	<entry>CATEGORY / SUBCATEGORY</entry>
        <entry>EMERGING FAR TERM</entry>
        <entry>Remarks</entry>
      </row>
<!--
      <row>
        <entry namest="c1" nameend="c4" align="left"><emphasis  role="bold"><xsl:text>Service Area : </xsl:text><xsl:call-template name="capitalize">
...
<xsl:template match="sp-view" mode="sa-children">
  <xsl:if test="./header or ./subclass or contains(./select/@mode, 'farterm')">
    <row>
      <entry/><entry/>
      <entry><xsl:apply-templates 
           select="./select[@mode='farterm']"/><xsl:apply-templates
           select="./select[@mode='farterm']/@id" mode="addindexentry"/></entry>
...
<xsl:template match="sp-view" mode="subarea-children">
  <xsl:if test="./header or ./subclass or ./select[@mode='farterm']">
    <row>
      <entry/>
      <entry><xsl:apply-templates 
           select="./select[@mode='farterm']"/><xsl:apply-templates
